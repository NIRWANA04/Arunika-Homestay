...
if (isset($_POST['submit'])) {
    ...
    $phone = $_POST['phone'];
    $email = $_POST['email'];
    $password = md5($_POST['password']); // Sebaiknya gunakan hashing yang lebih aman
    $jenis_kelamin = $_POST['jenis_kelamin'];
    $tanggal_lahir = $_POST['tanggal_lahir'];
    $pekerjaan = $_POST['pekerjaan'];
    $kota_asal = $_POST['kota_asal'];
    
    // Query untuk mengecek apakah tabel 'user' sudah ada di database
    $check_table_query = "SHOW TABLES LIKE 'user'";
    $result = mysqli_query($koneksi, $check_table_query);
    
    if (mysqli_num_rows($result) == 0) {
        // Jika tabel 'user' belum ada, buat tabel secara dinamis
        $create_table_query = "
            CREATE TABLE `user` (
    ...
    // Setelah tabel tersedia, simpan data ke dalam tabel 'user'
